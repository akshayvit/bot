    ...
    for i in range(1,25):
        ...
        for r,d,f in os.walk(path):
            for file in f:
                ...
                zcr.append(li.feature.zero_crossing_rate(at))
                rms.append(li.feature.rms(at))
                mfc.append(li.feature.mfcc(at)[:13])
                lpc.append(li.lpc(at,2))
                teo.append(tkeo(at))
                spc.append(li.feature.spectral_centroid(y=at,sr=sr))
                rlp.append(rastaplp(at))
                tar.append(file.split("-")[2])
        path=r"E:\\python\\Actor_"
    ftur=[np.array(zcr),np.array(rms),np.array(mfc),np.array(lpc),np.array(teo),np.array(spc),np.array(rlp)]
    names=["zcr","rms","mfcc","lpc","teo","spc","rlp"]
    di={}
    for i in range(len(ftur)):
        name=names[i]
        if(len(np.shape(ftur[i]))==1):
            if name not in di.keys():
                di[name]=[ftur[i]]
            else:
                di[name].append(ftur[i])
        elif(len(np.shape(ftur[i]))==2):
            for j in range(len(ftur[i])):
                name+=str(j+1)
                if name in di.keys():
                    di[name].append(ftur[i][j])
                else:
                    di[name]=[ftur[i][j]]
        else:
            for j in range(len(ftur[i])):
                name+=str(j+1)
                for k in range(len(ftur[i][j])):
                    if name not in di.keys():
                        di[name]=[ftur[i][j][k]]
                    else:
                        di[name].append(ftur[i][j][k])
    print("ZCR:"+str(len(zcr[0][0])))
    print("RMS:"+str(len(rms[0][0])))
    print("MFCC:"+str(len(mfc[0][0])))
    print("LPC:"+str(len(lpc[0])))
    print("TEO:"+str(len(teo[0])))
    print("SPC:"+str(len(spc[0][0])))
    print("RLP:"+str(len(rlp[0][0])))
    df=pd.DataFrame({"zcr":zcr,"rms":rms,"mfcc":mfc,"lpc":lpc,"teo":teo,"spc":spc,"rlp":rlp})
    df.to_csv("actors_emotions.csv",index=None,header=True)
